...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nombre) || empty($mensaje) || !$email) {
        echo "<script>alert('Por favor completa todos los campos y usa un email válido.'); window.history.back();</script>";
        exit;
    }
    ...
    try {
        ...
        $mail->Password   = 'onbd pwty wqge fskv'; // App password de Gmail
        ...
        if (!empty($_FILES['imagen1']['tmp_name'])) {
            ...
            if (in_array($mime1, $allowedTypes) && $_FILES['imagen1']['size'] <= $maxSize) {
                ...
                $error .= "Imagen 1: formato no permitido o tamaño excedido.\n";
            }
        }
        ...
        if (!empty($_FILES['imagen2']['tmp_name'])) {
            ...
            if (in_array($mime2, $allowedTypes) && $_FILES['imagen2']['size'] <= $maxSize) {
                ...
                $error .= "Imagen 2: formato no permitido o tamaño excedido.\n";
            }
        }

        // Si no hay errores de archivos, enviamos
        if (!$error) {
            $mail->isHTML(true);
